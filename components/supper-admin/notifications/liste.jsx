   ...
   console.log(users)
    useEffect(()=>{
      ...
      const fetchData = async () => {
          ...
          try{
            const res = await fetch('/supperadmin/api/notifications/liste')  
...
<div className="p-1 text-[-1xl]">
    ...
    <table className="table-responsive w-[100%] rounded">
        <thead>
          ...
          <th className="border w-1/6 px-2 py-2">province</th>
          <th className="border w-1/6 px-2 py-2">commune</th>
            <th className="border w-1/6 px-2 py-2">Expéditeur</th>
            <th className="border w-1/6 px-2 py-2">contact</th>
            <th className="border w-1/6 px-4 py-2">message</th>
            <th className="border w-1/6 px-4 py-2">Date de réception</th>
          </tr>
        </thead>
        <tbody>
          {data && (data.map((item,i=0)=>(
            ...
            <tr >
                ...
                <td className="border "> {item.nomprovince}</td>
                <td className="border "> {item.nomcommune} </td>
