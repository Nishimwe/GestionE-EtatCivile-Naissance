...
import MereEnfant from "./mere"
import { useSearchParams } from "next/navigation"
const PereEnfant=({precedent,data,dataenfant,closeModal})=>{
        ...
        const handleNationaliteChange=(e)=>{
          ...
          const FetchChald=async()=>{
            ...
            try {
              ...
              setNomPere(datas.result[0].pere)
              setProfessionPere(datas.result[0].professionpere)
              setNationalitePere(datas.result[0].nationalitepere)
              setAgePere(datas.result[0].agepere)
              setResidencePere(datas.result[0].residencepere)
    ...
    return(
      <>
       {OpenModalPere &&( <div
        className="flex justify-center ml-8 h-[30rem] bg-white border-4 p-2 border-black"
      >
      ...
      {OpenModalMere &&(<MereEnfant precedent={handlePrecedent} pere={pere} data={data} dataenfant={dataenfant} closeModal={closeModal}/>) }
      </>
    )
